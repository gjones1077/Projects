...
__global__ void compute_on_gpu(int *life, int *previous_life, int X_limit, int Y_limit) {
    ...
    for (int i = x0; i < X_limit; i += x_stride) {
        for (int j = y0; j < Y_limit; j += y_stride) {
            previous_life[(i+1)*(Y_limit+2) + (j+1)] = life[i*Y_limit + j];
        }
    }

    for (int i = x0; i < X_limit + 1; i += x_stride) {
        for (int j = y0; j < Y_limit + 1; j += y_stride) {
...
__global__ void padded_matrix_copy(int *dst, int *src, int width, int height, int padding) {
    ...
    for (int i = x0; i < width; i += x_stride) {
        for (int j = y0; j < height; j += y_stride) {

            const int new_i = i + padding;
            const int new_j = j + padding;
            dst[new_i*(height+padding*2) + new_j] = src[i*height + j];
...
/*
 * (SERIAL) Processes the life array for the specified number of iterations.
 */
