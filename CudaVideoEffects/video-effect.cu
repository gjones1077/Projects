...
/*  Set the block size of the kernel launches.
...
__global__ void convolveGPU(float const* in, float *out, int width, int height, float const* kernel, int kernelWidth, int kernelHeight) {
    ...
    // compute gpu thread indices and stride values
    const int x0 = blockIdx.x * blockDim.x + threadIdx.x + halfKernelHeight;
    const int x_stride = blockDim.x * gridDim.x;
    const int y0 = blockIdx.y * blockDim.y + threadIdx.y + halfKernelWidth;
    const int y_stride = blockDim.y * gridDim.y;
    /* point-wise loop over the image pixels */
    for (int i = x0; i < height-halfKernelHeight; i += x_stride) {
        for (int j = y0; j < width-halfKernelWidth; j += y_stride) {
